...
| `createNativeSocket` | proc ×2 | Создать новый сокет (перегрузки с перечислениями и cint) |
| `close` | proc | Закрыть и освободить сокет |
| `setInheritable` | proc | Управление наследованием сокета дочерними процессами |
| `bindAddr` | proc | Привязать сокет к адресу |
| `listen` | proc | Пометить сокет как принимающий соединения |
| `accept` | proc | Принять входящее соединение, вернуть дескриптор и IP клиента |
| `getAddrInfo` | proc ×2 | Разрешить имя хоста + порт в `AddrInfo` (нужно освободить!) |
| `toKnownDomain` | proc | Преобразовать сырой `cint` в `Option[Domain]` |
| `getServByName` | proc | Поиск сервиса по имени и протоколу |
| `getServByPort` | proc | Поиск сервиса по порту и протоколу |
| `getHostByName` | proc | Разрешить имя хоста в `Hostent` |
| `getHostByAddr` | proc | Обратное разрешение IP в `Hostent` |
| `getHostname` | proc | Имя локальной машины |
| `getProtoByName` | proc | Числовой код протокола по имени |
| `makeUnixAddr` | proc | Построить `Sockaddr_un` для пути к Unix-сокету |
| `ntohl` | proc | Сетевой→хостовый порядок байт, 32 бита |
| `ntohs` | proc | Сетевой→хостовый порядок байт, 16 бит |
| `htonl` | шаблон | Хостовый→сетевой порядок байт, 32 бита |
| `htons` | шаблон | Хостовый→сетевой порядок байт, 16 бит |
| `getSockOptInt` | proc | Прочитать целочисленный параметр сокета |
| `setSockOptInt` | proc | Записать целочисленный параметр сокета |
| `setBlocking` | proc | Переключить блокирующий/неблокирующий режим |
| `getSockDomain` | proc | Получить семейство адресов сокета |
| `getSockName` | proc | Получить номер порта привязки |
| `getLocalAddr` | proc | Получить кортеж (локальный адрес, порт) |
| `getPeerAddr` | proc | Получить кортеж (адрес, порт) удалённого узла |
| `getAddrString` | proc ×2 | Преобразовать `SockAddr*` в строку |
| `selectRead` | proc | Ожидать сокеты, готовые к чтению (обёртка select) |
| `selectWrite` | proc | Ожидать сокеты, готовые к записи (обёртка select) |
| `toInt` | proc ×3 | Преобразовать `Domain`/`SockType`/`Protocol` в `cint` |
| `toSockType` | proc | Отобразить `Protocol` на `SockType` |
