...
Этот справочник охватывает экспортируемые функции всех пяти модулей. Разделы упорядочены от самого простого (MD5) к самому защищённому (bcrypt) — так удобнее читать последовательно. Внутри каждого раздела функции расположены в порядке их типичного использования.
