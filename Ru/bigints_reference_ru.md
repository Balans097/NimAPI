...
Этот справочник охватывает экспортируемые функции обоих модулей. `bigints` реализует целые числа произвольной точности; `bigints/random` добавляет генерацию случайных `BigInt` через стандартный `std/random`.
